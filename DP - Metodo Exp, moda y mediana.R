...
cat(" Moda real:", as.character(mode(Causa)), "\n",
    "Ejemplos de Moda con ruido: ",
    as.character(sample(Res[,1], size = 1, prob = Res[,2])), ", ",
    as.character(sample(Res[,1], size = 1, prob = Res[,2])), "y ",
    as.character(sample(Res[,1], size = 1, prob = Res[,2]))
)
...
privacy_parameter <- 0.0001  # Parámetro de privacidad elegido
...
#C:\Users\victo\OneDrive - Benem?rita Universidad Aut?noma de Puebla\Documents\TESIS Ms Cs Mt\Differential Privacy Codigos y bases de datos\conjunto_de_datos_defunciones_registradas_2021_csv\conjunto_de_datos


Dataset <- read.csv("C:/Users/victo/OneDrive - Benem?rita Universidad Aut?noma de Puebla/Documents/TESIS Ms Cs Mt/Differential Privacy Codigos y bases de datos/conjunto_de_datos_defunciones_registradas_2021_csv/conjunto_de_datos/conjunto_de_datos_defunciones_registradas_2021.csv", header=FALSE)
...
# Funci?n para el Mecanismo Exponencial para la moda
exponential_mechanism_mode <- function(data, epsilon) {
  sensitivity <- 1  # Sensibilidad de la consulta de moda
  unique_values <- unique(data)
  
  # Calcula la puntuaci?n para cada valor ?nico
  scores <- sapply(unique_values, function(value) {
    count <- sum(data == value)
    return(count)
  })
  
  # Calcula la distribuci?n de probabilidad usando el Mecanismo Exponencial
  scaled_scores <- (epsilon * scores) / (2 * sensitivity)
  probabilities <- exp(scaled_scores) / sum(exp(scaled_scores))
